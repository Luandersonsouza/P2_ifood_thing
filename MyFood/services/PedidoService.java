...
public class PedidoService {
    ...
    public void adicionarProduto(int numeroPedido, int idProduto) {
        Pedido pedido = getPedido(numeroPedido, "Nao existe pedido em aberto");
    ...
    private Pedido getPedido(int numero) {
        return getPedido(numero, "Pedido nao encontrado");
    }

    private Pedido getPedido(int numero, String mensagemErro) {
        return db.getPedidos().stream()
                .filter(p -> p.getNumero() == numero)
                .findFirst()
                .orElseThrow(() -> new MyFoodException(mensagemErro));
    }
