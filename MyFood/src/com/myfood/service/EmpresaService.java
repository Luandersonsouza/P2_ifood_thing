...
public class EmpresaService {
    ...
    public int criarRestaurante(int idDono, String nome, String endereco, String tipoCozinha) {
        ...
        // Verifica se já existe empresa com mesmo nome (global)
        ...
        // Verifica se o mesmo dono já tem empresa com mesmo nome e endereço
        boolean mesmaEmpresa = db.getEmpresas().stream()
                .anyMatch(e -> e.getIdDono() == idDono && e.getNome().equals(nome) && e.getEndereco().equals(endereco));
        if (mesmaEmpresa) {
    ...
    public int criarMercado(int idDono, String nome, String endereco, String abre, String fecha, String tipoMercado) {
        ...
        Validador.validarNome(nome);
        Validador.validarEndereco(endereco);
        if (abre == null || fecha == null || tipoMercado == null)
            throw new IllegalArgumentException("Dados invalidos");

        // Verifica nome único global
        boolean nomeExiste = db.getEmpresas().stream().anyMatch(e -> e.getNome().equals(nome));
        if (nomeExiste) {
            throw new MyFoodException("Empresa com esse nome ja existe");
        }
    ...
    public int criarFarmacia(int idDono, String nome, String endereco, boolean aberto24Horas, int numeroFuncionarios) {
        ...
        Validador.validarNome(nome);
        Validador.validarEndereco(endereco);
        if (numeroFuncionarios < 0) throw new IllegalArgumentException("Numero de funcionarios invalido");
    ...
    public String getEmpresasDoUsuario(int idDono) {
        usuarioService.getUsuario(idDono);
        ...
        switch (atributo.toLowerCase()) {
            ...
            case "aberto24horas":
                if (empresa instanceof Farmacia) {
                    return String.valueOf(((Farmacia) empresa).isAberto24Horas());
                }
                throw new MyFoodException("Atributo invalido");
    ...
    public List<Integer> getEntregadores(int idEmpresa) {
        Empresa empresa = getEmpresa(idEmpresa);
        return new ArrayList<>(empresa.getEntregadores());
    }
    ...
    public void alterarFuncionamento(int idMercado, String abre, String fecha) {
        ...
        if (!(empresa instanceof Mercado)) {
            throw new MyFoodException("Empresa nao e um mercado");
        }
